...
const DISMISS_KEY = 'thuisbasis-install-dismissed';
const DISMISS_DURATION_MS = 7 * 24 * 60 * 60 * 1000; // 7 Tage

const INTERACTION_KEY = 'thuisbasis-install-interactions';
const INTERACTION_THRESHOLD = 2;

class ThuisBasisInstallPrompt extends HTMLElement {
  constructor() {
    super();
  ...
  _showBanner(isIOS) {
    ...
    style.textContent = `
      ...
      .banner {
        ...
        border: 1px solid var(--color-border, #e8e6e0);
      ...
      .subtitle {
        ...
        color: var(--color-text-secondary, #6b6a65);
      ...
      .btn-dismiss {
        ...
        cursor: pointer;
        color: var(--color-text-tertiary, #9b9a96);
        padding: 0;
        min-height: 32px;
        min-width: 32px;
        transition: background 0.15s ease;
      ...
      .btn-dismiss:hover {
        background: var(--color-surface-3, #eeecea);
      }
  ...
  async _onInstallClick() {
    ...
    try {
      ...
      console.log('[thuisbasis-install-prompt] Ergebnis:', result.outcome);

      if (result.outcome === 'accepted') {
        this._remove();
      }
    } catch (err) {
      console.error('[thuisbasis-install-prompt] Fehler:', err);
    }
...
customElements.define('thuisbasis-install-prompt', ThuisBasisInstallPrompt);
